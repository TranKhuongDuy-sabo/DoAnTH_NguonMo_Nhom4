...
<div class="container mb-5">
   ...
   <div class="row" id="product-list">
    ...
    foreach ($products as $row): 
        $count++;
        // Nếu là sản phẩm thứ 9 trở đi thì thêm class 'product-hidden' và 'd-none' (ẩn đi)
...
<script>
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            // 1. Tìm tất cả các sản phẩm đang bị ẩn
            const hiddenProducts = document.querySelectorAll('.product-hidden');
            
            // 2. Hiện chúng lên (bằng cách xóa class d-none của Bootstrap)
            hiddenProducts.forEach(product => {
                product.classList.remove('d-none');
                // Hiệu ứng fade-in nhẹ (tùy chọn)
